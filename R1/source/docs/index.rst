.. _docs:

Varnish Documentation
=====================

.. tabularcolumns:: l | l | l | l | l | l

.. list-table:: Documentation Links
    :widths: auto

    * - `8.0 </docs/8.0/>`__
      - `Installation </docs/8.0/installation/>`__
      - `Tutorial </docs/8.0/tutorial/>`__
      - `User-Guide </docs/8.0/users-guide/>`__
      - `Reference </docs/8.0/reference/>`__
      - Latest

    * - `7.7 </docs/7.7/>`__
      - `Installation </docs/7.7/installation/>`__
      - `Tutorial </docs/7.7/tutorial/>`__
      - `User-Guide </docs/7.7/users-guide/>`__
      - `Reference </docs/7.7/reference/>`__
      - Previous

    * - `6.0 </docs/6.0/>`__
      - `Installation </docs/6.0/installation/>`__
      - `Tutorial </docs/6.0/tutorial/>`__
      - `User-Guide </docs/6.0/users-guide/>`__
      - `Reference </docs/6.0/reference/>`__
      - LTS

    * - `Trunk </docs/trunk/>`__
      - `Installation </docs/trunk/installation/>`__
      - `Tutorial </docs/trunk/tutorial/>`__
      - `User-Guide </docs/trunk/users-guide/>`__
      - `Reference </docs/trunk/reference/>`__
      - Next

    * - `7.6 </docs/7.6/>`__
      - `Installation </docs/7.6/installation/>`__
      - `Tutorial </docs/7.6/tutorial/>`__
      - `User-Guide </docs/7.6/users-guide/>`__
      - `Reference </docs/7.6/reference/>`__
      - Deprecated

    * - `7.5 </docs/7.5/>`__
      - `Installation </docs/7.5/installation/>`__
      - `Tutorial </docs/7.5/tutorial/>`__
      - `User-Guide </docs/7.5/users-guide/>`__
      - `Reference </docs/7.5/reference/>`__
      - Deprecated

    * - `7.4 </docs/7.4/>`__
      - `Installation </docs/7.4/installation/>`__
      - `Tutorial </docs/7.4/tutorial/>`__
      - `User-Guide </docs/7.4/users-guide/>`__
      - `Reference </docs/7.4/reference/>`__
      - Deprecated

    * - `7.3 </docs/7.3/>`__
      - `Installation </docs/7.3/installation/>`__
      - `Tutorial </docs/7.3/tutorial/>`__
      - `User-Guide </docs/7.3/users-guide/>`__
      - `Reference </docs/7.3/reference/>`__
      - Deprecated

    * - `7.2 </docs/7.2/>`__
      - `Installation </docs/7.2/installation/>`__
      - `Tutorial </docs/7.2/tutorial/>`__
      - `User-Guide </docs/7.2/users-guide/>`__
      - `Reference </docs/7.2/reference/>`__
      - Deprecated

    * - `7.1 </docs/7.1/>`__
      - `Installation </docs/7.1/installation/>`__
      - `Tutorial </docs/7.1/tutorial/>`__
      - `User-Guide </docs/7.1/users-guide/>`__
      - `Reference </docs/7.1/reference/>`__
      - Deprecated

    * - `7.0 </docs/7.0/>`__
      - `Installation </docs/7.0/installation/>`__
      - `Tutorial </docs/7.0/tutorial/>`__
      - `User-Guide </docs/7.0/users-guide/>`__
      - `Reference </docs/7.0/reference/>`__
      - Deprecated

    * - `6.6 </docs/6.6/>`__
      - `Installation </docs/6.6/installation/>`__
      - `Tutorial </docs/6.6/tutorial/>`__
      - `User-Guide </docs/6.6/users-guide/>`__
      - `Reference </docs/6.6/reference/>`__
      - Deprecated

    * - `6.5 </docs/6.5/>`__
      - `Installation </docs/6.5/installation/>`__
      - `Tutorial </docs/6.5/tutorial/>`__
      - `User-Guide </docs/6.5/users-guide/>`__
      - `Reference </docs/6.5/reference/>`__
      - Deprecated

    * - `6.4 </docs/6.4/>`__
      - `Installation </docs/6.4/installation/>`__
      - `Tutorial </docs/6.4/tutorial/>`__
      - `User-Guide </docs/6.4/users-guide/>`__
      - `Reference </docs/6.4/reference/>`__
      - Deprecated

    * - `6.3 </docs/6.3/>`__
      - `Installation </docs/6.3/installation/>`__
      - `Tutorial </docs/6.3/tutorial/>`__
      - `User-Guide </docs/6.3/users-guide/>`__
      - `Reference </docs/6.3/reference/>`__
      - Deprecated

    * - `6.2 </docs/6.2/>`__
      - `Installation </docs/6.2/installation/>`__
      - `Tutorial </docs/6.2/tutorial/>`__
      - `User-Guide </docs/6.2/users-guide/>`__
      - `Reference </docs/6.2/reference/>`__
      - Deprecated

    * - `6.1 </docs/6.1/>`__
      - `Installation </docs/6.1/installation/>`__
      - `Tutorial </docs/6.1/tutorial/>`__
      - `User-Guide </docs/6.1/users-guide/>`__
      - `Reference </docs/6.1/reference/>`__
      - Deprecated

    * - `5.2 </docs/5.2/>`__
      - `Installation </docs/5.2/installation/>`__
      - `Tutorial </docs/5.2/tutorial/>`__
      - `User-Guide </docs/5.2/users-guide/>`__
      - `Reference </docs/5.2/reference/>`__
      - Late Middle Ages

    * - `5.1 </docs/5.1/>`__
      - `Installation </docs/5.1/installation/>`__
      - `Tutorial </docs/5.1/tutorial/>`__
      - `User-Guide </docs/5.1/users-guide/>`__
      - `Reference </docs/5.1/reference/>`__
      - High Middle Ages

    * - `5.0 </docs/5.0/>`__
      - `Installation </docs/5.0/installation/>`__
      - `Tutorial </docs/5.0/tutorial/>`__
      - `User-Guide </docs/5.0/users-guide/>`__
      - `Reference </docs/5.0/reference/>`__
      - Early Middle Ages

    * - `4.1 </docs/4.1/>`__
      - `Installation </docs/4.1/installation/>`__
      - `Tutorial </docs/4.1/tutorial/>`__
      - `User-Guide </docs/4.1/users-guide/>`__
      - `Reference </docs/4.1/reference/>`__
      - Antiquity

    * - `4.0 </docs/4.0/>`__
      - `Installation </docs/4.0/installation/>`__
      - `Tutorial </docs/4.0/tutorial/>`__
      - `User-Guide </docs/4.0/users-guide/>`__
      - `Reference </docs/4.0/reference/>`__
      - Ancient History

    * - `3.0 </docs/3.0/>`__
      - `Installation </docs/3.0/installation/>`__
      - `Tutorial </docs/3.0/tutorial/>`__
      - `User-Guide </docs/3.0/users-guide/>`__
      - `Reference </docs/3.0/reference/>`__
      - Bronze Age

    * - `2.1 </docs/2.1/>`__
      - `Installation </docs/2.1/installation/>`__
      - `Tutorial </docs/2.1/tutorial/>`__
      - `User-Guide </docs/2.1/users-guide/>`__
      - `Reference </docs/2.1/reference/>`__
      - Neolithic

For the historically inclined, Archive.org has a complete copy
of `the old Trac Wiki <https://web.archive.org/web/20150929070920/https://www.varnish-cache.org/trac>`__.

Documentation from Varnish Software
-----------------------------------

`Varnish Software <https://www.varnish-software.com/>`__ offers
documentation resources which also apply to the FOSS Version:

`The Varnish Book <https://book.varnish-software.com/>`__
`(& it's GitHub project) <https://github.com/varnish/Varnish-Book/>`__

In the Varnish Software Wiki:
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  `Wordpress <https://www.varnish-software.com/wiki/content/tutorials/wordpress/index.html>`__
-  `Drupal <https://www.varnish-software.com/wiki/content/tutorials/drupal/index.html>`__
-  `Magento 2 <https://www.varnish-software.com/wiki/content/tutorials/magento2/index.html>`__
-  `Adobe Experience Manager <https://www.varnish-software.com/wiki/content/tutorials/aem.html>`__
-  `Expression Engine <https://www.varnish-software.com/wiki/content/tutorials/expressionEngine.html>`__

-  `Varnish Web Developer FAQ <https://www.varnish-software.com/wiki/faq/index.html>`__
-  `Getting started with Varnish resources <https://www.varnish-software.com/wiki/start/index.html>`__
-  `Using Varnish on any site Guides and Tutorials <https://www.varnish-software.com/wiki/content/tutorials/varnish/index.html>`__

- `The Wiki's GitHub project <https://github.com/varnish/Varnish-Wiki/>`__

.. toctree::
	:hidden:
